...
for DATASET in USPTO_480kseed42 USPTO_480kseed2025
do
  # case LOAD_FROM
  PREFIX=${DATASET}_${MODEL}_${REPR_START}_${REPR_END}
  case "$DATASET" in
      USPTO_480kseed42)
          LOAD_FROM="./checkpoints/pretrained/USPTO_480k_dgat.pt"
          ;;
      USPTO_480kseed2025)
          LOAD_FROM="./checkpoints/pretrained/USPTO_480k_dgat.pt"
          ;;
      *)
          LOAD_FROM=""
          echo "LOAD_FROM is not set for $DATASET, using default empty value"
          ;;
  esac
  CUDA_VISIBLE_DEVICES=1 python train_shuffle.py \
    --model="$MODEL" \
    --data_name="$DATASET" \
    --task="$TASK" \
