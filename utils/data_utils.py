...
class G2SBatch:
    ...
    def to(self, device):
        ...
        if self.data_indices is not None:
            self.data_indices = self.data_indices.to(device)
    ...
    def pin_memory(self):
        ...
        if self.data_indices is not None:
            self.data_indices = self.data_indices.pin_memory()
...
class G2SDataset(Dataset):
    ...
    def update_data_indices(self, remove_indices, epoch):
        data_indices = copy.deepcopy(self.data_indices)
        data_indices = set(data_indices)
        remove_indices = set(remove_indices)
        left_indics = data_indices.difference(remove_indices)
        self.data_indices = list(left_indics)
        self.data_size = len(self.data_indices)
        print("The length of previous expert dataset is: {}".format(len(remove_indices)))
        print("The length of left dataset is: {}".format(len(left_indics)))
        # save remove_indices to file.txt related to epoch
        with open(f"./save_stere/expert_{epoch}_train_ids.txt", 'w') as f:
    ...
    def __getitem__(self, index: int) -> G2SBatch:
        ...
        data_indices = torch.tensor(data_indices, dtype=torch.long)
