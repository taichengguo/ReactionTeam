...
import pickle # save to pickle and merge all different results
...
def main(args):
    ...
    if args.do_predict and os.path.exists(args.result_file):
        ...
        # TODO apply lora
        if args.data_name == "USPTO_STEREO" or "0.33" in args.load_from or "0.66" in args.load_from or args.data_name == "USPTO_480kseed42":
            apply_lora_to_decoder(model, d_model=pretrain_args.decoder_hidden_size, rank=128, alpha=256, device=device)
        else:
            apply_lora_to_decoder(model, d_model=pretrain_args.decoder_hidden_size, rank=64, alpha=32, device=device)
        ...
        # TODO set dropout
        if args.do_dropout:
            # Force dropout to remain active
            ...
            for test_idx, test_batch in enumerate(test_loader):
                ...
                for predictions in results["predictions"]:
                    ...
                    for prediction in predictions:
                        predicted_idx = prediction.detach().tolist()
                        # TODO replace tokens with token_ids
                        predicted_tokens = [vocab_tokens[idx] for idx in predicted_idx[:-1]]
                        predicted_tokens2 = [str(i) for i in predicted_idx]

                        smi = " ".join(predicted_tokens)
                        smi2 = " ".join(predicted_tokens2)

                        smis.append(smi)
                        smis_token.append(smi2)

                    smis = ",".join(smis)
                    smis_token = ",".join(smis_token)

                    all_predictions.append(f"{smis}\n")
                    all_predictions2.append(f"{smis_token}\n")
                    # pdb.set_trace()

        # TODO Save predicting idx
        with open(args.result_file, "w") as of:
            of.writelines(all_predictions)

        result_file2 = args.result_file.replace(".txt", "_token.txt")
        with open(result_file2, "w") as of:
            of.writelines(all_predictions2)
